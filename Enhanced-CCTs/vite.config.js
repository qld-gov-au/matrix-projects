...
export default defineConfig({
  plugins: [
    {
      ...
      async closeBundle() {
        // Explicitly clear out old compiled js and css assets while preserving other dist assets
        ...
        for (const [folderName, bundle] of Object.entries(folderBundles)) {
          ...
          if (bundle.ts.length > 0) {
            ...
            // Write targets moved into dist/js/ folder with the embedded comment map
            const outputCodeWithMapLink = `${result.code}\n//# sourceMappingURL=${folderName}.js.map`;
            writeFileSync(`dist/js/${folderName}.js`, outputCodeWithMapLink);
            if (result.map) writeFileSync(`dist/js/${folderName}.js.map`, result.map);
          }
          ...
          if (bundle.scss.length > 0) {
            ...
            const cssResult = await esbuild.build({
              ...
              minify: true,
              sourcemap: true, 
              outfile: `dist/css/${folderName}.css`, // Output path targets dist/css/ folder
              plugins: [sassPlugin()],
  ...
  build: {
    outDir: 'dist',
    emptyOutDir: false, // STOP VITE FROM AUTOMATICALLY CLEARING THE DIST FOLDER
    minify: true,
    cssCodeSplit: true,
    sourcemap: true,
